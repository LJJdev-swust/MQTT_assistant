...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent), m_activeConnectionId(-1), m_titleLabel(nullptr),
    m_monitorCountLabel(nullptr), m_receivedMsgCount(0),
    m_toastLabel(nullptr), m_toastTimer(nullptr)
{
...
void MainWindow::setupContentArea(QWidget *content)
{
    ...
    connect(m_chatWidget, &ChatWidget::requestMoreMessages,
            this, &MainWindow::loadOlderMessagesAsync);
    ...
    // ── Corner widget: [消息计数 (居中)] [清除记录] ──
    QWidget *cornerWidget = new QWidget(m_tabWidget);
    QHBoxLayout *cornerLayout = new QHBoxLayout(cornerWidget);
    cornerLayout->setContentsMargins(4, 0, 4, 0);
    cornerLayout->setSpacing(6);

    m_monitorCountLabel = new QLabel("接收 0 条", cornerWidget);
    m_monitorCountLabel->setObjectName("monitorCountLabel");
    m_monitorCountLabel->setStyleSheet("color: #666666; font-size: 12px;");
    m_monitorCountLabel->setAlignment(Qt::AlignVCenter | Qt::AlignLeft);

    QPushButton *clearBtn = new QPushButton("清除记录", cornerWidget);
    clearBtn->setObjectName("btnClearChat"); // 这个对象名很重要，匹配QSS
    clearBtn->setToolTip("清除聊天记录");

    cornerLayout->addWidget(m_monitorCountLabel);
    cornerLayout->addStretch();
    cornerLayout->addWidget(clearBtn);
    ...
    connect(dlg, &QDialog::accepted, this, [this, dlg]()
            {
                ...
                if (config.name.isEmpty()) { showToast("连接名称不能为空"); return; }
                int id = m_db.saveConnection(config);
                if (id < 0) { showToast("保存连接失败"); return; }
                config.id = id;
...
void MainWindow::onEditConnection(int connectionId)
{
    ...
    ConnectionDialog *dlg = new ConnectionDialog(config, this);
    dlg->setAttribute(Qt::WA_DeleteOnClose);
    dlg->setWindowFlags(dlg->windowFlags() | Qt::Window);
    connect(dlg, &QDialog::accepted, this, [this, dlg, connectionId]()
            {
...
// 判断 payload 是否为纯十六进制字节序列（如 "7e00 0000 02b2 0300"）
// 规则：非空，且每个 token 都是 1-2 位十六进制字符，token 之间以空格/换行分隔
static bool looksLikeHexDump(const QString &s)
{
    if (s.trimmed().isEmpty())
        return false;
    const QStringList tokens = s.trimmed().split(QRegularExpression(R"(\s+)"), Qt::SkipEmptyParts);
    if (tokens.isEmpty())
        return false;
    static const QRegularExpression hexToken(R"(^[0-9A-Fa-f]{1,4}$)");
    for (const QString &t : tokens)
    {
        if (!hexToken.match(t).hasMatch())
            return false;
    }
    // 需要至少有 4 个 token 才认为是 HEX dump（避免把短数字误判）
    return tokens.size() >= 4;
}

// 统一的 dataType 检测，返回检测后的类型（不修改 payload 内容）
static MessageDataType detectDataType(const QString &payload)
{
    if (payload.startsWith("HEX: "))
        return Hex;
    if (looksLikeHexDump(payload))
        return Hex;
    QJsonParseError err;
    QJsonDocument::fromJson(payload.toUtf8(), &err);
    return (err.error == QJsonParseError::NoError) ? Json : Text;
}
...
void MainWindow::onConnectRequested(int connectionId)
{
    ...
    if (!m_clients.contains(connectionId))
    {
        ...
        connect(client, &MqttClient::connected, this, [this, connectionId]()
                {
                    ...
                    subscribeAllForConnection(connectionId); }, Qt::QueuedConnection);
        ...
        connect(client, &MqttClient::messageReceived, this, [this, connectionId](const QString &topic, const QString &payload, bool retained)
                {
                    if (m_activeConnectionId == connectionId) {
                        ...
                        if (!retained) {
                            ...
                            msg.dataType     = detectDataType(payload);
        ...
        connect(client, &MqttClient::errorOccurred, this, [this, connectionId](const QString &msg)
                {
                    m_connectionPanel->setLoading(connectionId, false);
                    showToast("错误：" + msg, 4000); }, Qt::QueuedConnection);
    }
    ...
    connect(dlg, &QDialog::accepted, this, [this, dlg]()
            {
                ...
                if (cmd.name.isEmpty()) { showToast("命令名称不能为空"); return; }
                cmd.connectionId = m_activeConnectionId;
                int id = m_db.saveCommand(cmd);
                if (id < 0) { showToast("保存命令失败"); return; }
                cmd.id = id;
                m_commands[id] = cmd;
...
void MainWindow::onEditCommand(int commandId)
{
    if (!m_commands.contains(commandId))
        return;
    CommandDialog *dlg = new CommandDialog(m_commands[commandId], this);
    dlg->setAttribute(Qt::WA_DeleteOnClose);
    dlg->setWindowFlags(dlg->windowFlags() | Qt::Window);
    connect(dlg, &QDialog::accepted, this, [this, dlg, commandId]()
            {
...
{
    ScriptDialog *dlg = new ScriptDialog(this);
    dlg->setAttribute(Qt::WA_DeleteOnClose);
    dlg->setWindowFlags(dlg->windowFlags() | Qt::Window);
    connect(dlg, &QDialog::accepted, this, [this, dlg]()
            {
                ScriptConfig script = dlg->config();
                if (script.name.isEmpty()) { showToast("脚本名称不能为空"); return; }
                script.connectionId = m_activeConnectionId;
                int id = m_db.saveScript(script);
                if (id < 0) { showToast("保存脚本失败"); return; }
                script.id = id;
                m_scripts[id] = script;
...
void MainWindow::onEditScript(int scriptId)
{
    if (!m_scripts.contains(scriptId))
        return;
    ScriptDialog *dlg = new ScriptDialog(m_scripts[scriptId], this);
    dlg->setAttribute(Qt::WA_DeleteOnClose);
    dlg->setWindowFlags(dlg->windowFlags() | Qt::Window);
    connect(dlg, &QDialog::accepted, this, [this, dlg, scriptId]()
            {
                                       ...
                                       bool outgoing, int connectionId, bool retained)
{
    ...
    if (!retained)
    {
        ...
        // Update received-message counter in-memory (no DB round-trip needed)
        if (!outgoing && connectionId == m_activeConnectionId)
        {
            ++m_receivedMsgCount;
            if (m_monitorCountLabel)
                m_monitorCountLabel->setText(
                    QString("接收 %1 条").arg(m_receivedMsgCount));
        }
    }
...
{
    static const int kMonitorMaxRows = 500; // 监控表最大行数

    // 暂时禁用排序
    bool sortingEnabled = m_monitorTable->isSortingEnabled();
    m_monitorTable->setSortingEnabled(false);

    // 超出上限时从顶部移除最旧的行，保持总行数不超过上限
    while (m_monitorTable->rowCount() >= kMonitorMaxRows)
        m_monitorTable->removeRow(0);
    ...
    QString timeStr = msg.timestamp.isValid()
                          ? msg.timestamp.toString("yyyy-MM-dd hh:mm:ss.zzz")
...
void MainWindow::onClearHistoryRequested(int connectionId)
{
    ...
    if (m_monitorCountLabel)
        m_monitorCountLabel->setText("接收 0 条");
    showToast("聊天记录已清除");
}
...
void MainWindow::loadMessagesAsync(int connectionId)
{
    ...
    m_chatWidget->showLoadingOverlay(); // show spinner immediately while DB query runs
    m_monitorTable->setRowCount(0);
    // Reset to 0 immediately; refreshReceivedCountAsync will fill the real DB value
    m_receivedMsgCount = 0;
    if (m_monitorCountLabel)
        m_monitorCountLabel->setText("接收 0 条");
    refreshReceivedCountAsync(connectionId);
                ...
                if (db.open())
                {
                    ...
                    if (q.exec())
                    {
                        while (q.next())
                        {
                            ...
                            {
                                // JSON 格式（与 saveAndDisplayMessage 写入时一致）
                                m.payload = payload;
                                m.dataType = Json;
                            }
                            else
                            {
                                // 普通文本（dataType==0），但仍尝试 JSON 解析
                                // 兼容旧数据：库中存 dataType=0 但内容实际为 JSON
                                QJsonParseError err;
                                QJsonDocument::fromJson(payload.toUtf8(), &err);
                                m.payload = payload;
                                m.dataType = (err.error == QJsonParseError::NoError) ? Json : Text;
                            }

                            m.outgoing = q.value(4).toBool();
                            m.retained = false;
                            m.timestamp = QDateTime::fromString(
                                q.value(5).toString(), Qt::ISODateWithMs);
                            // 兼容旧格式（不含毫秒）
...
void MainWindow::onMonitorRowDoubleClicked(int row, int /*col*/)
{
    ...
    // ── 元信息区域（QTextEdit 只读，内容可选中复制） ──
    QLabel *metaLabel = new QLabel("消息元信息:", dlg);
    layout->addWidget(metaLabel);

    QTextEdit *metaEdit = new QTextEdit(dlg);
    metaEdit->setReadOnly(true);
    metaEdit->setMaximumHeight(90);
    metaEdit->setFontFamily("Consolas, Monaco, Courier New, monospace");
    metaEdit->setPlainText(QString("时间:  %1\n方向:  %2\n主题:  %3")
                               .arg(time, dir, topic));
        ...
        for (int i = 0; i < bytes.size(); i += 16)
        {
            ...
            for (int j = 0; j < 16; ++j)
            {
                if (i + j < bytes.size())
                {
                    ...
                    line += (ok && byteVal >= 32 && byteVal <= 126) ? QChar(byteVal) : QChar('.');
                }
                else
                {
                    line += ' ';
    ...
    connect(bbox, &QDialogButtonBox::rejected, dlg, &QDialog::close);

    connect(copyBtn, &QPushButton::clicked, dlg, [dlg, payload]()
            {
                QApplication::clipboard()->setText(payload);
                const QString orig = dlg->windowTitle();
                dlg->setWindowTitle("消息详情 - 已复制原始数据");
                QTimer::singleShot(1200, dlg, [dlg, orig]() { dlg->setWindowTitle(orig); }); });

    connect(copyHexBtn, &QPushButton::clicked, dlg, [dlg, payload]()
            {
                QString data = payload.startsWith("HEX: ") ? payload.mid(5).trimmed() : payload;
...
void MainWindow::loadOlderMessagesAsync(int connectionId, int oldestId)
{
    if (m_activeConnectionId != connectionId)
        return;
    ...
    connect(watcher, &QFutureWatcher<QList<MessageRecord>>::finished, this,
            [this, connectionId, watcher]()
            {
                ...
                QList<MessageRecord> older = watcher->result();
                m_chatWidget->prependMessages(older);
            });

    QFuture<QList<MessageRecord>> future = QtConcurrent::run(
        [dbPath, connectionId, oldestId]() -> QList<MessageRecord>
        {
            const QString connName = QStringLiteral("mqtt_older_") +
                                     QUuid::createUuid().toString(QUuid::WithoutBraces);
            QList<MessageRecord> result;
            {
                QSqlDatabase db = QSqlDatabase::addDatabase("QSQLITE", connName);
                ...
                if (db.open())
                {
                    ...
                    q.prepare(
                        "SELECT id, connection_id, topic, payload, outgoing, timestamp, data_type "
                        "FROM messages WHERE connection_id=:connid AND id < :oldest "
                        "ORDER BY id DESC LIMIT 50");
                    q.bindValue(":connid", connectionId);
                    q.bindValue(":oldest", oldestId);
                    if (q.exec())
                    {
                        while (q.next())
                        {
...
// ──────────────────────────────────────────────
//  Received-message count (async DB query)
// ──────────────────────────────────────────────

void MainWindow::refreshReceivedCountAsync(int connectionId)
{
    if (connectionId < 0)
        return;
    ...
    connect(watcher, &QFutureWatcher<qint64>::finished, this,
            ...
            {
                watcher->deleteLater();
                // Only update the label if this connection is still active
                if (m_activeConnectionId != connectionId)
                    return;
                qint64 cnt = watcher->result();
                m_receivedMsgCount = cnt;
                if (m_monitorCountLabel)
                    m_monitorCountLabel->setText(
                        QString("接收 %1 条").arg(cnt));
            });

    QFuture<qint64> future = QtConcurrent::run(
        [dbPath, connectionId]() -> qint64
        {
            const QString connName = QStringLiteral("mqtt_count_") +
                                     QUuid::createUuid().toString(QUuid::WithoutBraces);
            qint64 result = 0;
            {
                QSqlDatabase db = QSqlDatabase::addDatabase("QSQLITE", connName);
